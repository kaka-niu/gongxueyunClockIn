...
def recognize_clickWord_captcha(target: str, wordlist: list) -> str:
    ...
    if os.environ.get('SAVE_CAPTCHA_SNAPSHOTS') == '1':
        ...
        logger.info(f"验证码验证前快照已保存: {snapshot_path}")

    bboxes = detect_objects("./models/yolov5n.onnx", image)

    # 识别每个文本框中的文本，并存储为列表以便处理重复字符
    recognized_list = []
    for bbox in bboxes:
        try:
            ...
            logger.info(f"识别到字符: {text}, 坐标: {bbox}, 中心: ({center_x}, {center_y})")
        except Exception as e:
            logger.warning(f"处理文本框时出错: {e}")

    logger.info(f"需要识别的字符列表: {wordlist}")
    logger.info(f"识别到的字符列表: {[item['text'] for item in recognized_list]}")
    ...
    for word in wordlist:
        # 查找所有匹配的字符
        matches = [item for item in recognized_list if item['text'] == word]
        
        if matches:
            # 如果有多个匹配，选择最左边的（按x坐标排序）
            if len(matches) > 1:
                logger.info(f"找到多个匹配的字符 '{word}': {len(matches)} 个")
                # 按x坐标排序，选择最左边的
                matches.sort(key=lambda x: x['center_x'])
            ...
            logger.info(f"成功匹配字符: {word}, 坐标: ({x}, {y}), 原始中心: ({center_x}, {center_y})")
        else:
            logger.warning(f"未找到字符: {word}")
            # 尝试模糊匹配
            found = False
            for item in recognized_list:
                if word in item['text'] or item['text'] in word:
                    logger.info(f"模糊匹配成功: {word} -> {item['text']}")
                    # 使用固定偏移量：x+10, y-10
                    ...
                    y = center_y - 10
                    random_coordinates.append({"x": x, "y": y})
                    logger.info(f"模糊匹配字符: {word}, 坐标: ({x}, {y}), 原始中心: ({center_x}, {center_y})")
                    found = True
                    break
            
            if not found:
                # 找不到字符，不凑合，抛出异常让上层重试
                raise ValueError(f"验证码识别失败: 无法识别字符 '{word}'，已识别: {[item['text'] for item in recognized_list]}")
    
    # 检查是否成功生成了所有坐标
    if len(random_coordinates) != len(wordlist):
        raise Exception(f"验证码识别不完整: 需要识别 {len(wordlist)} 个字符，但只识别了 {len(random_coordinates)} 个")
    
    logger.info(f"最终生成的坐标: {random_coordinates}")
